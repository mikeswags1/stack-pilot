...
// EVAL-SAFETY SWEEP (pre-July-20): live-verify the listings most likely to actually
// SELL â€” fresh (last 21 days, eBay promotes them), previously-sold ASINs, and thin
// margins. Ends confirmed rule-breakers only (explicit OOS / no Prime / at-loss /
// ROI<8%). Missing data never ends anything. Receipts: scripts/receipts/stale-ended.jsonl
// Run: node scripts/eval-safety-sweep.mjs [batch=700]
...
const BATCH = Math.max(50, Math.min(1200, Number(process.argv[2] || '700')))
...
// High-sale-probability set: fresh listings, ever-sold ASINs, thin margins.
const rows = await sql(`
  ...
  WHERE user_id = ANY($1::int[]) AND ended_at IS NULL AND ebay_listing_id <> ''
    AND (price_rechecked_at IS NULL OR price_rechecked_at < NOW() - INTERVAL '5 days')
    AND (
      listed_at > NOW() - INTERVAL '21 days'
      OR (amazon_price > 0 AND ebay_price > 0 AND amazon_price/ebay_price > 0.55)
      OR EXISTS (SELECT 1 FROM listed_asins s WHERE UPPER(s.asin)=UPPER(l.asin) AND s.ended_at IS NOT NULL)
    )
  ORDER BY listed_at DESC
  LIMIT $2`, [[1,3], BATCH])
console.log(`EVAL-SAFETY SWEEP: ${rows.length} high-sale-probability listings to live-verify`)

const parsePrice=s=>{const m=String(s||'').match(/([\d,]+\.\d{2})/);return m?parseFloat(m[1].replace(/,/g,'')):null}
let healthy=0, ended=0, unknown=0, credits=false, done=0
const receipts=[]
for(const r of rows){
  let d=null
  for(let a=0;a<2;a++){
  ...
  if(credits){ console.log('credit/rate stop'); break }
  ...
  if(reasons.length===0){ healthy++; if(done%50===0) console.log(`  ...${done}/${rows.length}`); continue }
  console.log(`END [${reasons.join(',')}] amz ${r.ap}->${price??'?'} ebay $${r.ep}  ${r.title.slice(0,45)}`)
  ...
  if(/<Ack>Success<\/Ack>|<Ack>Warning<\/Ack>/.test(tx)||/already|ended|closed|not found/i.test(tx)){
    ended++
    await sql(`UPDATE listed_asins SET ended_at=NOW(), amazon_status_reason=$2 WHERE ebay_listing_id=$1 AND ended_at IS NULL`,[r.ebay_listing_id,'evalsweep_'+reasons.join(',')])
  }
...
console.log(`\nRESULT: ${healthy} healthy, ${ended} ended, ${unknown} no-data (untouched) of ${done} checked`)
